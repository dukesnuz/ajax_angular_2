...
if (isset($_SESSION['build_load']['bid']) && isset($_SESSION['agent_id']['aid'])) {
    ...
    $query = "SELECT build_load_id FROM build_load_receivers WHERE build_load_id = :bid";
    ...
    if ($result && $check_flag == false) {
        ...
        try {
            // if in edit
            $query = "UPDATE build_load_receivers SET shipper_id = :sid WHERE build_load_id = :bid";
        ...
        try {
            $query = "INSERT INTO build_load_receivers (build_load_id, shipper_id)
                                VALUES(:bid, :sid)";
            $statement = $dba->prepare($query);
            $statement->bindValue(':bid', $bid);
